...
namespace GUI
{
	constexpr const char* g_ExtendedPlasmaColorMapID = "PLASMA##EXT";
	...
	//bool TreeNode(const void* ID, const ETreeNodeFlags flags = ETreeNodeFlags::None, const char* fro);
	//void TreePop();
	...
	bool MenuItem(const char* menuItemName);
	void EndMenu();
	...
	void PushTextColor(const float r, const float g, const float b, const float a);
	void PopTextColor();

	void SameLine(const float offsetX);
	void HorizontalSeparator();
	void VerticalSeparator();

	bool BeginCombinationBox(const char* label, const std::string& currentItem);
	bool CombinationBoxSelectable(const char* label, const bool isSelected);
	void EndCombinationBox();

	bool BeginPlot(const char* plotName);
	void SetupAxes(const char* xAxis, const char* yAxis, const bool xAutoFit, const bool yAutoFit);
	void SetupAxesLimits(const float xAxesMin, const float xAxesMax, const float yAxisMin, const float yAxisMax);
	void StackedBarPlot(const char** labels, const float* values, const uint32_t itemCount, const uint32_t groupCount, const float barSize);
	void StackedBarPlot(const char** labels, const uint32_t* values, const uint32_t itemCount, const uint32_t groupCount, const float barSize);

	void PieChart(const char** labels, const float* values, const uint32_t valueCount, const double x, const double y, const double radius);
	void EndPlot();

	bool ButtonCentered(const std::string_view label);
	void SearchBar(std::string& outFilter, const bool spanAvailableWidth, const std::string_view hint = "Search \xef\x80\x82");
}
...
#define ICON_FA_LINK_HORIZONTAL "\xef\x84\xa7"
